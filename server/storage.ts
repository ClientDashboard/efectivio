...
export class DatabaseStorage implements IStorage {
  ...
  async getAuditLogs(entityType?: AuditEntity, entityId?: string): Promise<AuditLog[]> {
    // Si se proporcionan el tipo de entidad y el ID, filtrar por ellos
    ...
    // Si solo se proporciona el tipo de entidad, filtrar solo por ese campo
    if (entityType) {
      return await db
        .select()
        .from(auditLogs)
...
export class MemStorage implements IStorage {
  ...
  async createJournalEntry(insertEntry: InsertJournalEntry, lines: InsertJournalLine[]): Promise<JournalEntry> {
    ...
    const entry: JournalEntry = {
      ...
      isAdjustment: insertEntry.isAdjustment || false
    };
    
    this.journalEntriesData.set(id, entry);
    
    // Create journal lines with the entry ID
