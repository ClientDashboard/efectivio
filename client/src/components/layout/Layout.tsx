...
export default function Layout({ children }: LayoutProps) {
  ...
  useEffect(() => {
    ...
    if (!isLoaded && !userId && location !== '/login' && location !== '/register' && !isDevelopment) {
      setLocation('/login');
    }
  }, [userId, isLoaded, location, setLocation, isDevelopment]);
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        {isDevelopment || userId ? <Sidebar /> : null} {/* Conditionally render Sidebar */}
